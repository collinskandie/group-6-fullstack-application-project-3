...
def get_favorites(current_user):
    # Only fetch favorites belonging to this user
    favorites = Favorite.query.filter_by(user_id=current_user.id).all()
    return jsonify([fav.to_dict() for fav in favorites]), 200


# 2. CREATE A FAVORITE (With ownership & validation)
...
def create_favorite(current_user):
    ...
    # Extra Validation: Avoid duplicates for this specific user
    ...
    try:
        new_fav = Favorite(
            ...
            user_id=current_user.id,  # Tied safely to the logged-in user.
...
# 3. UPDATE A FAVORITE (Strict ownership check)
@favorite_bp.route("/<int:id>", methods=["PUT"])
@token_required
def update_favorite(current_user, id):
    # Fetch favorite only if it belongs to current_user
...
# 4. DELETE A FAVORITE (Strict ownership check)
